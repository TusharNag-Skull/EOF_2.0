...
// ===== HEADING LETTER REVEAL — SAFE ADD ONLY =====
(function() {

  // Wait for everything else to load first
  window.addEventListener('load', function() {

    // Small delay so all other animations 
    // initialize first
    setTimeout(function() {

      // SAFE selector — only target specific 
      // section h2 headings, NOT hero, NOT navbar
      const safeHeadings = document.querySelectorAll(
        '#portfolio h2, #services h2, ' +
        '#about h2, #contact h2, ' +
        '#identity h2, #our-identity h2'
      );

      if (safeHeadings.length === 0) {
        // If no headings found with above selectors,
        // find all h2 EXCEPT inside hero and navbar
        const allH2 = document.querySelectorAll('h2');
        allH2.forEach(h => {
          const inHero = h.closest(
            '#hero, .hero-section, .hero, nav, .navbar'
          );
          if (!inHero) safeHeadings2.push(h);
        });
      }

      // Process each heading safely
      safeHeadings.forEach(function(heading) {

        // Skip if already processed
        if (heading.dataset.letterDone === 'true') return;

        // Skip if inside hero or navbar — double safety
        if (heading.closest('#hero, .hero-section, nav, .navbar')) return;
        ...
        const originalFont = window.getComputedStyle(heading).fontFamily;
        ...
        heading.innerHTML = originalText
          ...
          .map(function(char) {
            ...
            return '<span class="eof-letter" style="' +
      ...
      const revealObserver = new IntersectionObserver(
        function(entries) {
          entries.forEach(function(entry) {
            ...
            // Use GSAP if available — smoother
      ...
      // Observe all processed headings
      safeHeadings.forEach(function(heading) {
        if (heading.dataset.letterDone === 'true') {
          revealObserver.observe(heading);
        }
      });

    }, 1000); // 1 second delay — safety buffer
  });

})();
// ===== END HEADING LETTER REVEAL =====
